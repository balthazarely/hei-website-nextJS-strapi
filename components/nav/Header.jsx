import { useState } from "react";
import { useRouter } from "next/router";
import { Box, Flex, Text, Button, Stack } from "@chakra-ui/react";
import Logo from "./Logo";
import Menu from "./Menu";
import Link from "next/link";
import styles from "../styles/Header.module.css";

const NavBar = (props) => {
  const [isOpen, setIsOpen] = useState(false);
  const toggle = () => setIsOpen(!isOpen);

  return (
    <>
      <NavBarContainer {...props}>
        <Logo
          // w="100px"
          color={["primary.100", "primary.100", "primary.100", "primary.100"]}
        />
        <MenuToggle toggle={toggle} isOpen={isOpen} />
        <MenuLinks isOpen={isOpen} />
      </NavBarContainer>
      <Menu isOpen={isOpen} setIsOpen={setIsOpen} />
    </>
  );
};

const MenuLinks = ({ isOpen }) => {
  const router = useRouter();

  console.log(router.pathname);

  return (
    <Box display={{ sm: "none", md: "block" }}>
      <Stack spacing={8} direction={["row"]} pt={[4, 4, 0, 0]}>
        <MenuItem
          fontSize="sm"
          to="/"
          style={{ cursor: "pointer" }}
          className={router.pathname == "/" ? styles.activeLink : styles.link}
        >
          HOME
        </MenuItem>
        <MenuItem
          fontSize="sm"
          to="/about"
          style={{ cursor: "pointer" }}
          className={
            router.pathname == "/about" ? styles.activeLink : styles.link
          }
        >
          ABOUT
        </MenuItem>
        <MenuItem
          fontSize="sm"
          to="/contact"
          style={{ cursor: "pointer" }}
          className={
            router.pathname == "/contact" ? styles.activeLink : styles.link
          }
        >
          CONTACT
        </MenuItem>
        <MenuItem
          fontSize="sm"
          to="/portfolio"
          style={{ cursor: "pointer" }}
          className={
            router.pathname == "/portfolio" ? styles.activeLink : styles.link
          }
        >
          PORTFOLIO
        </MenuItem>
      </Stack>
    </Box>
  );
};

const NavBarContainer = ({ children, ...props }) => {
  return (
    <Flex
      as="nav"
      align="center"
      justify="center"
      w="100%"
      position="absolute"
      // mb={8}
      p={8}
      bg={["primary.500", "primary.500", "transparent", "transparent"]}
      color={["white", "white", "primary.700", "primary.700"]}
      className={styles.navcontainer}
      {...props}
    >
      <Flex
        align="center"
        justify="space-between"
        w="100%"
        className={styles.navcontentcontainer}
      >
        {children}
      </Flex>
    </Flex>
  );
};

const CloseIcon = () => (
  <svg width="24" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
    <title>Close</title>
    <path
      fill="white"
      d="M9.00023 7.58599L13.9502 2.63599L15.3642 4.04999L10.4142 8.99999L15.3642 13.95L13.9502 15.364L9.00023 10.414L4.05023 15.364L2.63623 13.95L7.58623 8.99999L2.63623 4.04999L4.05023 2.63599L9.00023 7.58599Z"
    />
  </svg>
);

const MenuIcon = () => (
  <svg
    width="24px"
    viewBox="0 0 20 20"
    xmlns="http://www.w3.org/2000/svg"
    fill="white"
  >
    <title>Menu</title>
    <path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v-2z" />
  </svg>
);

const MenuToggle = ({ toggle, isOpen }) => {
  return (
    <Box
      display={{ base: "block", md: "none" }}
      onClick={toggle}
      style={{ zIndex: 101 }}
    >
      {isOpen ? <CloseIcon /> : <MenuIcon />}
    </Box>
  );
};

const MenuItem = ({ children, isLast, to = "/", active, ...rest }) => {
  return (
    <Link href={to}>
      <Text
        display="block"
        {...rest}
        // className={` ${active ? styles.activeLink : styles.active}`}
      >
        {children}
      </Text>
    </Link>
  );
};

export default NavBar;
